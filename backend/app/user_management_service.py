...
class UserManagementService:
    ...
    def get_users(
        ...
        order_by: str = "last_request_time",
        order_dir: str = "DESC",
    ) -> Dict[str, Any]:
        """
        获取用户列表
        ...
        if where_clauses:
            ...
            base_sql_pg += " AND " + " AND ".join(where_clauses)
        ...
        if activity_filter:
            ...
            elif activity_filter == ActivityLevel.NEVER:
                having_clause = " HAVING MAX(l.created_at) IS NULL"
        ...
        try:
            ...
            # 尝试 MySQL 语法
            try:
                full_sql = base_sql + group_by + having_clause + order_clause_mysql + limit_clause
                result = self._db.execute(full_sql, params)
            except Exception:
                # PostgreSQL 语法
                full_sql = base_sql_pg + group_by_pg + having_clause + order_clause + limit_clause
                result = self._db.execute(full_sql, params)
    ...
    def batch_delete_inactive_users(
        ...
        Args:
            activity_level: 要删除的活跃度级别
            dry_run: 是否为预演模式（只统计不删除）
        ...
        try:
            ...
            if activity_level == ActivityLevel.VERY_INACTIVE:
                ...
                find_sql = """
                    ...
                    HAVING MAX(l.created_at) < :cutoff OR MAX(l.created_at) IS NULL
                """
            ...
            if dry_run:
                return {
                    ...
                    "dry_run": True,
                    "count": len(user_ids),
                    "users": usernames[:20],  # 只返回前 20 个用户名
